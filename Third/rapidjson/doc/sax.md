...
struct MyHandler {
    ...
    bool std::string(const char* str, SizeType length, bool copy) { 
        cout << "std::string(" << str << ", " << length << ", " << boolalpha << copy << ")" << endl;
        return true;
    }
...
class Handler {
    ...
    bool std::string(const Ch* str, SizeType length, bool copy);
    bool StartObject();
...
The `Reader` uses UTF-8 as both source and target encoding. The source encoding means the encoding in the JSON stream. The target encoding means the encoding of the `str` parameter in `std::string()` calls. For example, to parse a UTF-8 stream and outputs UTF-16 string events, you can define a reader by:

~~~~~~~~~~cpp
GenericReader<UTF8<>, UTF16<> > reader;
~~~~~~~~~~

Note that, the default character type of `UTF16` is `wchar_t`. So this `reader`needs to call `std::string(const wchar_t*, SizeType, bool)` of the handler.
...
There are two `std::string()` and `Key()` overloads. One is the same as defined in handler concept with 3 parameters. It can handle string with null characters. Another one is the simpler version used in the above example.
...
2. `Writer::std::string()` can handle string escaping (e.g. converting code point `U+000A` to `\n`) and Unicode transcoding.
...
The `SourceEncoding` template parameter specifies the encoding to be used in `std::string(const Ch*, ...)`.
...
struct MessageHandler
    ...
    bool std::string(const char* str, SizeType length, bool) {
        switch (state_) {
...
struct CapitalizeFilter {
    ...
    bool std::string(const char* str, SizeType length, bool) { 
        ...
        return out_.std::string(&buffer_.front(), length, true); // true = output handler need to copy the string
    }
    bool StartObject() { return out_.StartObject(); }
    bool Key(const char* str, SizeType length, bool copy) { return std::string(str, length, copy); }
